...
#read training data
xtrain<-read.csv(file.choose(), header=TRUE)
...
model_xgboost<-xgb.train(param, xgmat, nround, watchlist)
...
#read test data
xtest<-read.csv(file.choose(), header=TRUE)
...
outdata<-data.frame("EventId"=idx,
                    "RankOrder"=rorder,
                    "Class"=plabel)
write.csv(outdata, file="xgboost.pred.csv", quote=FALSE, row.names=FALSE)
